...
def download_file(url, dl_path):
  try:
    ...
    try:
      ...
      return True, os.path.join(f"{DOWNLOAD_LOCATION}/{filename}")
    except HTTPError:
      return False, error
...
def utube_dl(link):
  ytdl_opts = {
    'outtmpl' : os.path.join(DOWNLOAD_LOCATION, '%(title)s'),
    'noplaylist' : True,
    'logger': LOGGER,
  ...
  with youtube_dl.YoutubeDL(ytdl_opts) as ytdl:
    try:
      ...
      return False, str(e)
    for path in glob.glob(os.path.join(DOWNLOAD_LOCATION, '*')):
