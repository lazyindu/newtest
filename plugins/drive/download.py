...
from helpers.sql_helper import gDriveDB, idsDB
from helpers.utils import CustomFilters, humanbytes
from helpers.downloader import download_file, utube_dl
from helpers.gdrive_utils import GoogleDrive
from info import DOWNLOAD_LOCATION, LOGGER
from info import Messages, BotCommands
    ...
    if not message.media:
        ...
        if 'drive.google.com' in link:
            ...
            if '|' in link:
                ...
                link = link.strip()
                filename.strip()
                dl_path = os.path.join(f'{DOWNLOAD_LOCATION}/{filename}')
            else:
                link = link.strip()
                filename = os.path.basename(link)
                dl_path = DOWNLOAD_LOCATION
            LOGGER.info(f'Download:{user_id}: {link}')
            sent_message.edit(Messages.DOWNLOADING.format(link))
            result, file_path = download_file(link, dl_path)
            if result == True:
                sent_message.edit(Messages.DOWNLOADED_SUCCESSFULLY.format(
...
def _telegram_file(client, message):
    ...
    sent_message.edit(Messages.DOWNLOAD_TG_FILE.format(
        file.file_name, humanbytes(file.file_size), file.mime_type))
    LOGGER.info(f'Download:{user_id}: {file.file_id}')
    try:
        file_path = message.download(file_name=DOWNLOAD_LOCATION)
        sent_message.edit(Messages.DOWNLOADED_SUCCESSFULLY.format(
            os.path.basename(file_path), humanbytes(os.path.getsize(file_path))))
